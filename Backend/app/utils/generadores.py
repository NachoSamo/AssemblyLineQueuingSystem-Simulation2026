...
class FlujoAleatorio:
    ...
    Normalmente, pedirle números aleatorios a NumPy uno por uno en un bucle de simulación
    es costoso por la sobrecarga de comunicación entre Python y el motor de C de NumPy.

    Un flujo representa **un stream independiente**. El motor usa dos por réplica
    uno para ensamble y otro para cocción para que los procesos de ensamble y
    cocción no interfieran entre sí.
    """
...
def uniforme_con_rnd(
    ...
    Consume exactamente un RND, igual que `uniforme()`: el orden y
...
def resolver_semilla(semilla: int | None) -> int:
    ...
    reproducible después de haberse ejecutado.
...
def derivar_flujos_de_replica(
    ...
    Sirve para analizar la fila exacta dentro de la simulacion usando la semilla de la misma
    ...
    Del subárbol de la réplica se derivan dos hijos, uno por tipo de tiempo, para que 
    los flujos de ensamble y cocción no salgan del mismo stream intercalado.
